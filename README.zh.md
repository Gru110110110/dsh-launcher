...
可以在经过校验的市场快照中搜索、筛选、查看、安装和卸载 Cordis 与 Skill 插件。来源或兼容性存在疑问时，可以查看提醒并明确确认后继续安装。多包安装命令会将整组软件包安装到指定的 profile；卸载时按安装记录整组处理，并保留共享包与原有配置。安装前会展示确切来源、版本、目标位置、兼容性结论和执行风险；卸载时保留可恢复备份。如果 Harness 更新后因已安装插件不兼容而启动失败，启动器会通过可恢复卸载重试，并明确告知被移除的插件。
