...
- **M0 — state contract:** one reducer maps top-level Harness session events onto exactly five public states (`waiting`, `error`, `working`, `thinking`, `idle`). User questions and approvals become waiting, only reasoning stream chunks become thinking, all other active phases (including model text, tool-call generation, command execution, and searches) become working, failures become error, and completed/aborted turns become idle. Across concurrent sessions the priority is waiting → error → working → thinking → idle; subagents never replace the top-level display.
...
- **M3 — pet window:** Tauri creates a separate transparent, undecorated, always-on-top window. The React renderer lazy-loads Lottie and uses a one-slot latest-state queue: each live animation completes at least one cycle, duplicate or obsolete pending states coalesce, transitions occur only at cycle boundaries, and an unchanged state repeats without reloading. Reduced-motion and manual-preview modes still update immediately. The renderer keeps animation, bubble, CSS state, and accessible label synchronized; the window remains draggable, saves and clamps its physical position, and hides whenever Harness is not ready. Click-through applies to the whole window and is always reversible from the main Desktop Pet page.
