...
市场会从受支持的 `dsh plugin … add …` 命令解析完整 npm 包列表及 `--profile`（`pnpm add …` 默认使用 `web`）。README 中的不同命令仍视为替代方案：优先选择首条包含当前目录插件名的命令，否则选择首条有效命令。本地路径、Git 目标、Shell 命令链、未知选项及不安全的 profile 名称会被拒绝，不会只安装其中一部分。每个包分别校验 npm 仓库来源和 Cordis peer；确认凭据绑定完整包名、版本列表及 profile。共用的 `@deepseek-ai/dsh-base` 和 `@deepseek-ai/dsh-web-app` 绑定到 `deepseek-ai/deepseek-harness`。缺少仓库声明或仓库不匹配会作为可确认的提醒展示，不再永久阻止安装。预检直接列出各包来源提醒和兼容性问题，用户明确确认强制安装后可继续；registry 请求失败、包版本未完整解析时仍需重试。确认记录绑定每个包的版本、目标配置及提醒内容，来源信息变更后必须重新确认。强制安装同样禁用生命周期脚本，并保留事务回滚。

整组包通过一次禁用生命周期脚本的 pnpm 调用安装，新 bundle 层保留命令中的顺序，已有依赖和基础层保持不变。缺失的 profile 只在候选目录按已发布 Harness 模板初始化（自定义 profile 包含 `@deepseek-ai/dsh-base`、空 patch 和 hoisted pnpm 配置）。持久化的“原先不存在”标记让回滚能够撤回新建 profile，同时保留被拒绝目录。确认框列出全部包和目标 profile；自定义配置通过 `dsh --profile NAME` 启动，不自动切换启动器默认 Web 服务。profile 内的安装记录保存每个市场插件的包组，以及由市场新增的直接依赖。即使市场目录随后变更，一键卸载也按原记录整组事务处理；共享包、仍被依赖的包及原先已存在的包会保留。此前由市场新增而暂时保留的依赖，会在最后一个依赖组卸载时清退。确认框分别列出移除和保留的包。没有安装记录的旧版安装继续按明确选择的单包卸载，不推测所有权。配置与用户数据保留；回滚快照及被拒绝的 profile 用于恢复，卸载不清空包管理器下载缓存。

每个 profile 的持久化事务记录关联候选发布、备份及待验证日志。发布中断时恢复原 profile 和日志，并持久化撤回完成标记，确保候选清理再次中断后仍可重复恢复。发布成功后保留 last-good 快照直到验证完成。候选校验覆盖整组卸载依赖、辅助非 bundle 包、已有包的实际版本，以及候选准备期间用户对配置文件的修改。默认 Web 服务仅确认或回滚 `web`；自定义 profile 需要用户执行 `dsh --profile NAME` 验证，再明确确认验证或恢复待验证批次。清理备份前会校验确认绑定的待验证日志仍是当前批次。强制确认会关闭严格 peer 检查，继续禁用生命周期脚本；新增直接依赖保存为精确版本。
